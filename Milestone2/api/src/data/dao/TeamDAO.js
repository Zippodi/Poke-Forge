      ...
      try {
        ...
        for (const r of teamResults) {
          ...
          if (entryResults.length === 0) {
            reject("Error fetching team data - No pokemon entries found");
          }
          ...
          for (let e of entryResults) {
            ...
            if (e.item_id) {
              ...
              if (!itemResults[0]) {
                reject(`Error fetching team data - Could not get item with id ${e.item_id} `);
              }
            ...
            if (moveResults.length < 1 || moveResults.length > 4) {
              reject("Error fetching team data - Could not get correct move data");
            }
            ...
            if (typeResults.length === 0) {
              reject(`Error fetching team data - Could not get types for ${e.pokemon_name}`);
            }
            ...
            if (moveTypeResults.length != moveNames.length) {
              reject(`Error fetching team data - Error getting types of moves pokemon "${e.pokemon_name}" knows`);
            }
            pokemonDataArr.push({
              name: e.pokemon_name,
  ...
  return new Promise((resolve, reject) => {
    ...
    connection.getConnection(async (err, conn) => {
      try {
        ...
        if (teamResults.length === 0) {
          reject("Team could not be found or did not have permission to view");
        }
        //get pokemon entry info (not moves/items yet)
        let sql = "SELECT pe.id, p.id AS pokemon_id, p.name AS pokemon_name, a.name AS ability_name, pe.item_id AS item_id FROM pokemon_entry AS pe JOIN pokemon AS p ON p.id = pe.pokemon_id ";
        sql += "JOIN ability AS a ON a.id = pe.ability_id WHERE pe.team_id = ?";
          ...
          for (const p of team.pokemon) {
            try {
              ...
              if (abilResults.length === 0) {
                return conn.rollback(() => { reject(`Ability ${p.ability} is not valid on ${p.name}, or one of these values does not exist`); });
              }
              ...
              if (p.item) {
                ...
                if (itemResults.length === 0) {
                  return conn.rollback(() => { reject(`Item ${p.item} on ${p.name} is not a valid item`); });
                }
